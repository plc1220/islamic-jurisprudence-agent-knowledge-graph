...
## Anonymous conversation history (2026-09-16)

Sembang now supports Sembang baharu and Sejarah sembang. A versioned browser-local archive stores separate messages, citations, graph snippets and drafts under random conversation IDs. Existing server session chat is migrated when no local archive exists; local history subsequently takes precedence over the 24-hour server session. Switching is disabled while a response streams, so answers cannot land in another conversation. Only the selected conversation supplies chat context. History has no account or cross-device sync and is lost when browser site data is cleared; unavailable/full storage shows an explicit warning. The current server session remains a short-lived cache, not the history source of truth.
