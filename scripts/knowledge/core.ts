...
export function validateExtraction(raw: any, document: Document, section: Section): SectionResult {
  ...
  const claims: EvidenceClaim[] = raw.claims.map((item: any) => {
    ...
    if (relative < 0) throw new Error(`Evidence quote is absent from the source section: ${JSON.stringify(claim.quote.slice(0, 160))}. Copy a contiguous passage exactly, including punctuation, whitespace and Markdown.`);
