...
export type UpdatePhase = 'queued' | 'dispatch-unknown' | 'crawl' | 'extract' | 'publish' | 'complete' | 'failed';
export type UpdateState = { runId: string; phase: UpdatePhase; requestedAt: string; updatedAt: string; expectedActive: string | null; operation?: string; execution?: string; error?: string };
...
export async function runUpdate(store: CloudStore, runId: string, execute: (stage: 'crawl' | 'extract' | 'publish', state: UpdateState) => Promise<void>, execution?: string) {
  ...
  try {
    for (const phase of ['crawl','extract','publish'] as const) {
      state = await changeUpdate(store,runId,{phase,...(execution ? {execution} : {}),error:''});
    ...
    await changeUpdate(store,runId,{phase:'failed',error:String(error.message || 'Update failed').slice(0,500)});
    throw error;
  }
}
