...
import { ArrowLeft, ArrowRight, BookOpen, Download, ExternalLink, Search, LayoutGrid, List } from 'lucide-react';
...
export function Library() {
  ...
  useEffect(() => {
    ...
    const url = selectedId ? `/api/library/${encodeURIComponent(selectedId)}` : `/api/library?${new URLSearchParams({ q: search, source, page: String(page), pageSize: String(pageSize) })}`;
  ...
  }, [search, source, page, pageSize, selectedId, retry]);
  ...
  return <section className="space-y-6">
    ...
    {!selectedId && <div className="library-toolbar">
      <label className="flex items-center gap-2 text-sm text-stone-600">Artikel setiap halaman
        <select aria-label="Artikel setiap halaman" value={pageSize} onChange={event => { setPageSize(Number(event.target.value)); setPage(0); }}>
          {[12, 24, 48, 96].map(size => <option key={size} value={size}>{size}</option>)}
        </select>
      </label>
      <div role="group" aria-label="Paparan pustaka" className="flex gap-2">
        <Button aria-pressed={view === 'cards'} variant={view === 'cards' ? 'primary' : 'secondary'} onClick={() => setView('cards')}><LayoutGrid className="h-4 w-4" />Kad</Button>
        <Button aria-pressed={view === 'list'} variant={view === 'list' ? 'primary' : 'secondary'} onClick={() => setView('list')}><List className="h-4 w-4" />Senarai</Button>
      </div>
    </div>}
    {loading ? <div role="status" className="empty-panel">Memuatkan pustaka…</div> : error ? <div role="alert" className="empty-panel"><p>{error}</p><Button onClick={() => setRetry(n => n + 1)}>Cuba lagi</Button></div> : selectedId && article ?
      ...
      <p className="text-sm text-stone-500">{articles.length ? `Artikel ${page * pageSize + 1}–${page * pageSize + articles.length} · ` : ''}Halaman {page + 1}</p>
      {articles.length === 0 ? <div className="empty-panel">Tiada artikel sepadan. Cuba kata kunci atau sumber lain.</div> : <div className={view === 'cards' ? 'library-grid' : 'library-list'}>{articles.map(item => <button className="article-card" key={item.document_id} onClick={() => setSelectedId(item.document_id)}><span className="eyebrow">{item.source_name || 'Sumber lain'}</span><h3>{item.title}</h3><p>{item.excerpt}</p><span className="article-action">Baca artikel <ArrowRight /></span></button>)}</div>}
      <nav aria-label="Halaman pustaka" className="flex flex-wrap items-center justify-between gap-3"><Button disabled={page === 0} onClick={() => setPage(n => n - 1)}><ArrowLeft /> Sebelumnya</Button><Button disabled={!hasMore} onClick={() => setPage(n => n + 1)}>Seterusnya <ArrowRight /></Button></nav>
    </>}
  </section>;
}
